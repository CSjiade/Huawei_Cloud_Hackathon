...
router.get("/login", (request, response) => {
    ...
    const query = "SELECT id, password, name, email, details,tokens, if(isAdmin=1,  'true', 'false') as isAdmin FROM user WHERE email = ?";
    const args = [email]
    database.query(query, args, (error, result) => {
        if(error) throw error
        if(result.length == 1){
...
router.post("/register",uploadImage.single('image'), (request, response) => {
    ...
    const userdetail = request.body.details
    ...
    database.query(checkQuery, email , (error, result)  => {
        ...
        if(result.length != 0){
            ...
            const query = "INSERT INTO user(name, email, password, gender, age, image, details) VALUES(?, ?, ?, ?, ?,?,?)"
                
            // Encrypt Password
            bcrypt.hash(password, 10, (error, hashedPassword) => {
                if(error) throw error
        
                const args = [name, email, hashedPassword, gender, age, filePath, userdetail]
...
router.put("/upload", checkAuth, uploadImage.single('image'), (request, response) => {
    ...
    var localpath = "storage_user/"  + file.filename
